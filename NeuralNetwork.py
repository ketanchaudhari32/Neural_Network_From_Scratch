...
class NeuralNetwork():
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        for i in range(self.n_out):
            #########################################
            if np.isscalar(targets):
                output_deltas[i] = (outputs[i] - targets) * sigmoid_derivative(self.y_out[i])
            else:
                output_deltas[i] = (outputs[i] - targets[i]) * sigmoid_derivative(self.y_out[i])
            ########################################/
        ...
        for i in range(len(hidden_deltas)):
            ...
            for k in range(len(outputs)):
                try:
                    sum_weight +=  output_deltas[k] * self.w_hidden[k][i] 
                except:
                    pass
                
            hidden_deltas[i] = sigmoid_derivative(self.y_hidden[i])*(sum_weight)
            ########################################/
